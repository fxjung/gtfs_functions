...
def aggregate_speed(speeds, segments_gdf):
    ...
    data = (
        ...
        .sort_values(
            by=["route_id", "direction_id", "window", "stop_sequence"], ascending=True
        )
    )
...
def add_all_lines_speed(data, speeds, segments_gdf):
    ...
    # Clean data
    data_complete = data_complete[~data_complete.route_name.isnull()].reset_index(
        drop=True
    )
...
def add_free_flow(speeds, data_complete):
    ...
    max_speed_segment = speeds.pivot_table(
        "speed_kmh", index="segment_name", aggfunc="max"
    )

    max_speed_segment.rename(
        columns={"speed_kmh": "segment_max_speed_kmh"}, inplace=True
    )
...
def add_all_lines(line_frequencies, segments_gdf, labels, cutoffs):
    ...
    data_all_lines["min_per_trip"] = (
        (end_time - start_time) * 60 / data_all_lines.ntrips
    ).astype(int)
...
def window_creation(stop_times, cutoffs):
    ...
    if max(cutoffs) <= 24:
        stop_times["departure_time"] = fix_departure_time(
            stop_times.departure_time.values
        )
...
def add_frequency(
    ...
    trips_agg = stop_times.pivot_table(
        "trip_id", index=index_list, aggfunc="count"
    ).reset_index()
    ...
    trips_agg["min_per_trip"] = (
        (end_time - start_time) * 60 / trips_agg.ntrips
    ).astype(int)
