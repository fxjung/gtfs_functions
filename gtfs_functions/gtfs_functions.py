...
class Feed:
    ...
    def trips(self):
        ...
        if self._patterns and self._trips_patterns is None:
            (trips_patterns, routes_patterns) = self.get_routes_patterns(self._trips)
            self._trips_patterns = trips_patterns
            self._routes_patterns = routes_patterns
            return self._trips_patterns
        elif self._patterns:
    ...
    def get_dates(self):
        ...
        if start_date is not None:
            ...
            logging.info(
                "Start date is None. You should either specify a start date or set busiest_date to True."
            )
            return []
    ...
    def get_routes_patterns(self, trips):
        ...
        def version_hash(x):
            hash = hashlib.sha1(
                f"{x.route_id}{x.direction_id}{str(x.zipped_stops)}".encode("UTF-8")
            ).hexdigest()
            return hash[:18]

        trips_with_stops["pattern_id"] = trips_with_stops.apply(
            version_hash, axis=1, result_type="reduce"
        )
        ...
        route_patterns = trips_with_stops.pivot_table(
            ...
            index=[
                ...
                "direction_id",
                "shape_id",
                trips_with_stops.zipped_stops.astype(str),
            ],
            aggfunc="count",
        ).reset_index()

        route_patterns = (
            route_patterns.rename({"trip_id": "cnt_trips"}, axis=1)
            .sort_values(
        ...
        n_patterns = route_patterns.pivot_table(
            "cnt_trips", index=["route_name", "direction_id"], aggfunc="count"
        ).reset_index()
        n_patterns["route_pattern"] = n_patterns.cnt_trips.apply(
            lambda row: tuple(np.arange(1, row + 1))
        )
        ...
        n_patterns["pattern_name"] = (
            n_patterns.route_name
            + " - "
            + n_patterns.direction_id.astype(int).astype(str)
            + " - "
            + n_patterns.route_pattern
        )
        n_patterns.sort_values(
            by=["route_name", "direction_id", "route_pattern"], inplace=True
        )
        ...
        trips_with_stops = trips_with_stops.merge(
            route_patterns[["pattern_id", "route_pattern", "pattern_name"]], how="left"
        )
    ...
    def get_busiest_service_id(self):
        ...
        return (
            trips.pivot_table("trip_id", index="service_id", aggfunc="count")
    ...
    def parse_calendar(self):
        ...
        if calendar is not None:
            ...
            calendar["all_dates"] = calendar.apply(
                lambda x: np.array(
                    [
                        d
                        for d in pl.interval(x.start_date_dt, x.end_date_dt).range(
                            "days"
                        )
                    ]
                ),
            ...
            def get_hash_weekdays(row):
                return {i: v for i, v in enumerate(row[cols].values[0])}

            hash_weekdays = calendar.groupby("service_id").apply(get_hash_weekdays)

            # Filter dates depending on the days of the week
            calendar["filtered_dates"] = calendar.apply(
                lambda row: row.all_dates[
                    [hash_weekdays[row.service_id][d.weekday()] for d in row.all_dates]
                ],
                axis=1,
            )
        ...
        if calendar_dates is not None:
            ...
            calendar_dates["date_str"] = (
                calendar_dates.date.astype(str).apply(pl.parse).dt.date.astype(str)
            )
            ...
            cdates_hash = (
                ...
                .apply(lambda x: dict(zip(x, [True] * len(x))))
                .to_dict()
            )
        ...
        if busiest_date:
            ...
            for i, d in enumerate(dates):
                if d not in gtfs_dates:
                    ...
                    print(
                        f'The date "{d}" does not have service in this feed and will be removed from the analysis.'
                    )
    ...
    def get_trips(self):
        ...
        if (self.busiest_date) | (dates != []):
            ...
            if self.busiest_date:
                ...
                logging.info(
                    f"The busiest date/s of this feed or your selected date range is/are:  {busiest_date} with {max_trips} trips."
                )
                logging.info(
                    "In the case that more than one busiest date was found, the first one will be considered."
                )
            ...
            trips = (
                trips.set_index("service_id")
                .join(
                    dates_service_id[dates_service_id.date.isin(dates)].set_index(
                        "service_id"
                    ),
                    how="inner",
                )
                .reset_index(names="service_id")
                .drop(["keep", "date"], axis=1)
                .drop_duplicates()
            )
    ...
    def get_stop_times(self):
        ...
        stop_times["arrival_time"] = [
            seconds_since_midnight(t) if t not in [None, np.nan] else None
    ...
    def get_shapes(self):
        if self.geo:
            ...
            shapes = gpd.GeoDataFrame(
                data=shapes.index, geometry=shapes.values, crs=4326
            )
    ...
    def get_stops_freq(self):
        ...
        stop_frequencies = add_frequency(
            stop_times, labels, index_="stop_id", col="window", cutoffs=cutoffs
        )
        ...
        stop_frequencies = stop_frequencies.merge(stops[stops_cols], how="left")

        if self.geo:
            stop_frequencies = gpd.GeoDataFrame(
                data=stop_frequencies, geometry=stop_frequencies.geometry
            )
    ...
    def get_lines_freq(self):
        ...
        line_frequencies = add_frequency(
            stop_times_first,
            labels,
            index_=["route_id", "route_name", "shape_id"],
            col="window",
            cutoffs=cutoffs,
        )
        ...
        if self.geo:
            ...
            line_frequencies = gpd.GeoDataFrame(
                data=line_frequencies, geometry=line_frequencies.geometry, crs=4326
            )
    ...
    def get_segments(self):
        ...
        df_shape_stop["cut_distance_stop_point"] = df_shape_stop[
            ["geometry_stop", "geometry_shape"]
        ].apply(lambda x: x[1].project(x[0], normalized=True), axis=1)
        logging.info("Interpolating stops onto shape...")
        df_shape_stop["projected_stop_point"] = df_shape_stop[
            ["geometry_shape", "cut_distance_stop_point"]
        ...
        df_shape["list_of_points"] = df_shape.geometry.apply(
            lambda x: list(MultiPoint(x.coords).geoms)
        )
        df_shape_exp = df_shape.explode("list_of_points")
        df_shape_exp["projected_line_points"] = df_shape_exp[
            ["geometry", "list_of_points"]
        ].apply(lambda x: x[0].project(x[1], normalized=True), axis=1)
        ...
        df_shape_exp = df_shape_exp[
            ["shape_id", "list_of_points", "projected_line_points"]
        ]
        df_shape_exp.rename(
            {
        ...
        cuts[["end_stop_id", "end_stop_name"]] = cuts.groupby("shape_id")[
            ["stop_id", "stop_name"]
        ].shift(-1)
        ...
        for shape_id in cuts.shape_id.drop_duplicates():
            ...
            for i, cut in enumerate(cut_idx[:-1]):
                segment_geometries.append(
                    LineString(gdf.iloc[cut_idx[i] : cut_idx[i + 1] + 1].geometry)
                )

        # create into gpd adding additional columns
        segment_df = cuts.dropna(subset="end_stop_id", axis=0)
        logging.info(
            f"segments_df: {len(segment_df)}, geometry: {len(segment_geometries)}"
        )
        ...
        segment_gdf["segment_id"] = (
            segment_gdf.stop_id.astype(str)
            + " - "
            + segment_gdf.end_stop_id.astype(str)
        )
        segment_gdf["segment_name"] = (
            segment_gdf.stop_name + " - " + segment_gdf.end_stop_name
        )
    ...
    def get_avg_speeds(self):
        ...
        # Do we want a geodataframe?
        if self.geo:
            data = gpd.GeoDataFrame(data=data, geometry=data.geometry, crs=4326)
    ...
    def get_segments_freq(self):
        ...
        line_frequencies = add_frequency(
            stop_times,
            labels,
            index_=["route_id", "route_name", "stop_id"],
            col="window",
            cutoffs=cutoffs,
        )
        ...
        data_complete = add_all_lines(line_frequencies, segment_gdf, labels, cutoffs)
    ...
    def get_distance_between_stops(self):
        ...
        utm_stops = stops_[
            ["stop_index", "stop_id", "stop_name", "hex", "geometry"]
        ].to_crs(code(stops_))
        ...
        dist_df = pd.DataFrame(
            data=stops_comb, columns=["stop_index_1", "stop_index_2"]
        )
        ...
        dist_df["connection_time_min"] = dist_df.distance_m * walking_speed_ms / 60
...
def extract_file(file, feed):
    ...
    # check if the zip file came from a zipped folder
